...
static std::string ParseLogicalType(TInput &in)
{
    ...
    while (true)
    {
        ...
        {
        case 1:
        {                       // STRING (StringType - empty struct)
            SkipStruct(in);     // Just skip the empty StringType struct
            result = "varchar"; // Use varchar for STRING type
            break;
        }
        case 2:
        { // MAP (MapType - empty struct)
            SkipStruct(in);
            result = "map";
            break;
        }
        case 3:
        { // LIST (ListType - empty struct)
            SkipStruct(in);
            result = "array";
            break;
        }
        case 4:
        { // ENUM (EnumType - empty struct)
            SkipStruct(in);
            result = "enum";
            break;
        }
            ...
            while (true)
            {
                ...
                { // unit
                    ...
                    while (true)
                    {
                        ...
                        else if (unit_fh.id == 2)
                        { // MICROSECONDS
                            SkipStruct(in);
                            unit = "us";
                        }
                        else if (unit_fh.id == 3)
                        { // NANOSECONDS
                            SkipStruct(in);
                            unit = "ns";
                        }
        ...
        case 15:
        {                   // FLOAT16 (Float16Type - empty struct)
...
static SchemaElement ParseSchemaElement(TInput &in)
{
    ...
    while (true)
    {
        ...
        case 6:
        { // converted_type (legacy logical type)
...
static void ParseColumnMeta(TInput &in, ColumnStats &cs)
{
    ...
    while (true)
    {
        ...
        case 8:
        { // key_value_metadata: list<struct>; skip
            auto lh = ReadListHeader(in);
            for (uint32_t i = 0; i < lh.size; i++)
            {
                int16_t kv_last = 0;
                            ...
                            std::unordered_map<std::string, std::string> &out)
{
    ...
    if (elem.logical_type == "struct")
    {
        ...
        {
            // Root "schema" node: walk children so we see real columns (identifier, etc.)
        ...
        {
            // Non-root struct columns collapse to jsonb
            out[CanonicalizeColumnName(path)] = "jsonb";
            // return;
        }
    }
    ...
    if (elem.logical_type == "array")
    {
        ...
        if (!elem.children.empty())
        {
            ...
            while (cur)
            {
                if (!cur->logical_type.empty() &&
                    cur->logical_type != "struct" &&
    ...
    {
        out[path] = "fixed_len_byte_array[" + std::to_string(elem.type_length) + "]";
    }
...
static inline bool IsDotPrefixedAncestor(const std::string &ancestor, const std::string &leaf)
{
    // ancestor must be a proper prefix of leaf on a dot boundary: "a.b" is ancestor of "a.b.c"
    return leaf.size() > ancestor.size() && std::memcmp(leaf.data(), ancestor.data(), ancestor.size()) == 0 && leaf[ancestor.size()] == '.';
}

static void ApplyLogicalTypes(FileStats &fs,
        ...
        for (auto &col : rg.columns)
        {
            ...
            if (it == logical_type_map.end())
            {
                for (const auto &kv : logical_type_map)
                {
                    ...
                    if (key.size() > col.name.size() && key.compare(key.size() - col.name.size(), col.name.size(), col.name) == 0 && key[key.size() - col.name.size() - 1] == '.')
                    {
            ...
            // 4) jsonb ancestor propagation (only if no mapping found)
            {
            ...
            {
                col.logical_type = col.logical_type.empty() ? "binary" : col.logical_type;
            }
            else if (col.physical_type == "fixed_len_byte_array")
            {
                // Prefer to normalize this in InterpretSchema; otherwise leave as physical fallback.
